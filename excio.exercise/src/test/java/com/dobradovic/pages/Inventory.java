...
public class Inventory {
    ...
    private int countItemsInCart = 0;
    ...
    public void assertValidSortedItems(ElementsCollection elements) {
        if (elements.texts().equals(headlines)) {
            ...
            for (int j = 0; j < prices.size(); j++) {
                if (Double.parseDouble(elements.get(j).getText().substring(1)) == prices.get(j)) {
    ...
    public void assertShoppingCartCount() {
        if(shoppingCartBadge.exists()) {
            shoppingCartBadge.shouldHave(Condition.text("" + countItemsInCart));
            System.out.println("There is/are " + countItemsInCart + " item(s) in the cart" );
        }
        else {
